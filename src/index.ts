...
const configFile: PathLike = path.resolve(__dirname,"..", "config", "data.json");
...
class CmdLineParser {
  ...
  constructor() {
    this.args = yargs(process.argv.slice(2)).options({
      ...
      logLevel: {
        alias: 'l',
        choices: ["none", "error", "standard", "info", "debug", "trace"],
        default: "standard",
      ...
      startDate: {
        ...
        description: 'Date to start scrapping pictures (first of a given month)'
      },
      endDate: {
        alias: 'e',
        type: 'string',
        default: null,
        description: 'Date to end scrapping pictures (first of a given month)'
      },
      year: {
        alias: 'y',
        type: 'boolean',
  ...
  public async check(): Promise<boolean> {
    ...
    if (this.endDate == null) {
      this.setEndDate(new DoDate(null))
        .incMonth();
    }
  ...
  public getStartDate(): DoDate {
    if (this.startDate == null)
      return this.setStartDate(new DoDate(null));
