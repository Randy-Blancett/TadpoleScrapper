...
export default class OutputHandler {
    ...
    public getPathFromMime(mime: MimeType, date: Date, attatchment: number): string {
        return this.getImagePath(path.resolve(this.baseDir, mime.getExtention()), date, attatchment, mime.getExtention());
    }
    ...
    getImagePath(baseDir: string, date: Date, attatchment: number, ext: string): string {
        let filename = this.getFileName(date, attatchment, ext);
        let output: string = path.resolve(baseDir, filename);
        if (date == null) {
            Logger.error("Time is null for the event so it will be placed in base folder.");
            return output;
        }
        switch (this.split) {
            case SplitType.YEAR: {
                output = path.resolve(baseDir,
