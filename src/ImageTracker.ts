...
export default class ImageTracker {
   ...
   public  getFile4Mime(mime: MimeType): string|undefined {
    ...
    getDescription(): string {
        ...
        if (this.event.location !== null && this.event.location.summary !== null)
            output += this.event.location.summary + " ";
        if (this.event.time !== null)
            output += "(" + this.event.time.toLocaleDateString('en-us', { day: "numeric", month: "short", year: "numeric" }) + ")";
    ...
    getCity() {
        if (this.event === undefined || this.event.location === null)
            return "";
    ...
    getSubjects(): string[] {
        if (this.event === null || this.event.subjects === null)
            return [];
        return this.event.subjects;
    }
    getKeywords(): string[] {
        if (this.event === null || this.event.labels === null)
            return [];
